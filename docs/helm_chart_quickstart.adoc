  ...
  template:
    ...
    spec:
      ...
      - name: "mariadb"
        ...
        readinessProbe:
          exec:
            command: ["sh", "-c", "exec mysql -h 127.0.0.1 -u $MARIADB_USER -D $MARIADB_DATABASE -p $MARIADB_PASSWORD" -e 'SELECT 1'"]
